...
function createMaster(host){
  ...
  host.querySelector('[data-finalize]').addEventListener('click',()=>{if(drawing&&drawn.length<3){status.textContent='Select at least three vertices before finalizing the drawn path.';return;}sharedPath=copy(draft);try{localStorage.setItem(STORAGE,JSON.stringify(sharedPath));}catch{}window.dispatchEvent(new CustomEvent('eng654-shared-path'));stamp(host,sharedPath,analysis);status.textContent=`Saved and carried forward. ${summary(analysis)} Every panel reports the actual outcome, including when its requested case is absent.`;});
