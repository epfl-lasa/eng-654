...
async function createPumaLab(host,mode){
  ...
  host.innerHTML='<div class="l7-stage"><div class="hud">PUMA 560 · numerical wrist-center continuation · q₄–q₆ fixed</div></div><aside class="l7-panel"><div class="l7-controls"></div><div class="l7-plot"><h3>workspace path · x,z</h3><canvas></canvas></div><div><div class="l7-readout"></div><p class="l7-status"></p></div></aside>';
...
function createMaster(host){
  host.classList.add('l7-master');host.innerHTML='<div class="l7-plot"><h3>idealized D–H workspace path · ρ,z</h3><canvas></canvas></div><aside class="l7-master-side"><div class="l7-shared-badge">All atlas case-study slides use this finalized path.</div><div data-ranges></div><div class="l7-controls"><button class="primary" data-finalize>Finalize shared path</button><button data-reset>Reset</button></div><div class="l7-readout"></div><p class="l7-status"></p></aside>';
  ...
  host.querySelector('[data-finalize]').addEventListener('click',()=>{const analysis=analyzeCustomPath(draft);if(!analysis.regular.length||analysis.infeasible.length<2||!analysis.nonsingular.length){status.textContent='Not finalized: adjust the atlas loop until the preview includes a closed lift, two branch-infeasible lifts, and a nonsingular-change lift.';return;}sharedPath={...draft};localStorage.setItem('eng654-l7-square',JSON.stringify(sharedPath));analysisCache.clear();window.dispatchEvent(new CustomEvent('eng654-shared-path'));status.textContent='Finalized. Every lift panel now uses these exact idealized D–H vertices.';draw();});
  ...
  function refresh(){const analysis=analyzeCustomPath(draft);readout.textContent=pathText(draft);status.textContent=`Atlas preview: ${analysis.regular.length} closed lift(s) · ${analysis.infeasible.length} branch-infeasible lift(s) · ${analysis.nonsingular.length} nonsingular change(s). Numerical root searches may miss candidates.`;draw();}
  function draw(){if(!pair.ready)return;drawWorkspace(pair,draft,null,0,true);}refresh();
}

async function createCustomLab(host,mode){
  host.classList.add('l7-custom-lab');host.innerHTML='<div class="l7-custom-panels"><div class="l7-plot"><h3>idealized D–H workspace · shared square</h3><canvas data-work></canvas></div><div class="l7-plot"><h3>idealized joint-space lift · q₂,q₃</h3><canvas data-joint></canvas></div><div class="l7-stage"><div class="hud">course URDF/STL · posture illustration only</div></div></div><div class="l7-custom-controls"><div class="l7-controls"><button class="primary" data-play>Play</button><button data-reset>Reset</button></div><div><div class="l7-state-pills"></div><progress class="l7-progress" max="1" value="0"></progress><p class="l7-status"></p></div><div class="l7-readout"></div></div>';
  ...
  function describe(){const closure=result.closure??Infinity,min=result.minDet??0;if(mode==='custom-regular')status.textContent=`Regular atlas lift: workspace and joint paths close · joint closure ${closure.toFixed(3)} rad · min |det Jₚ| ${min.toFixed(3)}.`;else if(mode.includes('infeasible'))status.textContent=`Branch-infeasible lift ${mode.endsWith('a')?'A':'B'}: continuation reaches its closest singular configuration on edge ${edgeAt(result.failIndex||result.minIndex)} · min |det Jₚ| ${min.toExponential(2)}.`;else status.textContent=`Nonsingular change in the atlas: workspace closure 0 · joint endpoint separation ${closure.toFixed(3)} rad · min |det Jₚ| ${min.toFixed(3)}.`;}
