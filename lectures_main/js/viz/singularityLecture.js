...
async function loadUrdfStlVisuals(parent,urdfText,modelRoot) {
  ...
  await Promise.all(specs.map(async(spec)=>{const response=await fetch(new URL(spec.file,modelRoot));if(!response.ok)throw new Error(`Could not load course STL ${spec.file}.`);const geometry=parseStlGeometry(await response.arrayBuffer()),holder=new THREE.Group();holder.matrixAutoUpdate=false;const mesh=new THREE.Mesh(geometry,new THREE.MeshStandardMaterial({color:spec.color,roughness:.63,metalness:.05,transparent:true,opacity:.82,depthWrite:false,side:THREE.DoubleSide}));mesh.userData.isCourseStl=true;mesh.scale.fromArray(spec.scale);holder.add(mesh);group.add(holder);visuals.push({...spec,holder});}));
...
function course3RDet(q,a1){const c2=Math.cos(q[1]),s3=Math.sin(q[2]),c3=Math.cos(q[2]);return .75*(3*c3+4)*(c2*(c3-2*s3)-a1*s3);}
function symbolicPreferential6R(q,parameters){const [,q2,q3,q4,q5]=q,{a1,a2,a3,alpha2,d4}=parameters,s2=Math.sin(q2),c2=Math.cos(q2),s3=Math.sin(q3),c3=Math.cos(q3),s4=Math.sin(q4),c4=Math.cos(q4),s5=Math.sin(q5),c5=Math.cos(q5),sa=Math.sin(alpha2),ca=Math.cos(alpha2);return[
