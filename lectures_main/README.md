...
Open `exercises/exercise_02.html` for the 13-slide iiwa 7 IK tutorial. Students fix original q₃ and derive the D-H and PoE models in their working notes. Slide 3 provides derivation prompts without completed model tables or transforms; its confirmation button records that students have completed their derivations. The positioning-model panel also asks students to derive the reduced chain. Students then identify the home wrist point and choose Paden–Kahan after inspecting the robot geometry. Step 05 gives no geometric hints; the PK3 distance construction is introduced only after the correct choice. Correct answers unlock the next step. A wrong method choice links to the condition on Lecture 02 slide 30 in a new tab.
