...
(async()=>{
 ...
 try{
  ...
  await derivation.fill(text);await page.locator('#record-bonus').click();
  ...
  const legacy=structuredClone(key);for(const name of Object.keys(legacy.answers))if(name.startsWith('bonus.J.'))delete legacy.answers[name];
  ...
  console.log('PASS: 36 accessible blank cells; partial answers and derivation survive reload, download and upload; instructor matrix imports; legacy schema 1 remains compatible; no automatic bonus grading; responsive matrix.');
