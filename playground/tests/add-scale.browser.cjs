 ...
 * node playground/tests/add-scale.browser.cjs
 */
...
(async () => {
  ...
  try {
    ...
    const cell=(row,column,value)=>change('input[aria-label="Matrix row '+row+' column '+column+'"]',value,'input');
    const input=(label,value)=>change('[aria-label="'+label+'"]',value);
    await change('#example','empty');
    const a=await add('column-vector');
    for(let i=1;i<=3;i++)await cell(i,1,i);
    const b=await add('column-vector');
    for(let i=1;i<=3;i++)await cell(i,1,i+3);
    const sum=await add('add');
    await input('A · first vector',a);await input('B · vector to add',b);
    near(await output(),[[5],[7],[9]]);
    const scale=await add('scale');
    assert.equal(await run('document.querySelector("#bindings")'),null);
    assert.equal(await run('document.querySelector("#block-units").hidden'),true);
    await change('input[aria-label="Scalar factor"]','-2','input');
    await input('A · vector or matrix',sum);
    near(await output(),[[-10],[-14],[-18]]);
    await change('input[aria-label="Scalar factor"]','\\lambda_{2}','input');
    assert.equal(await run('document.querySelector(".math-input-preview msub mi").textContent'),'λ');
    await change('input[aria-label="Scalar factor"]','0.5','input');
    near(await output(),[[2.5],[3.5],[4.5]]);
    await select(b);await input('Rows (m)',2);await select(sum);
    assert.match(await run('document.querySelector("#inspector-error").textContent'),/same length and orientation/);
    await click('#undo');await select(scale);near(await output(),[[2.5],[3.5],[4.5]]);
    for(const id of [a,b]){await select(id);await change('#vector-orientation','row');}
    await select(scale);near(await output(),[[2.5,3.5,4.5]]);
    await change('input[aria-label="Scalar factor"]','0','input');near(await output(),[[0,0,0]]);
    await change('input[aria-label="Scalar factor"]','0.5','input');
    await click('#save-function');await click('#function-submit');
    await click('.library-block');const saved=(await state()).selected;await select(saved);near(await output(),[[2.5,3.5,4.5]]);
    await key('e');near(await output(),[[2.5,3.5,4.5]]);
    await select(scale);await click('#output-python');
    assert.match(await run('document.querySelector("#python-code").textContent'),/0\.5/);
    await click('#code-dialog form button');
    ...
    assert.deepEqual((await state()).graph,before);
    await select(scale);near(await output(),[[2.5,3.5,4.5]]);
    await click('#arrange-fit');await click('#toggle-output');
    await run('new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)))');
    const shot=await page.send('Page.captureScreenshot',{format:'png'});fs.writeFileSync('/tmp/playground-add-scale.png',Buffer.from(shot.data,'base64'));
    assert.deepEqual(page.errors,[]);
    console.log('PASS: add/scale palette, named inputs, numeric and LaTeX factors, blank defaults, row/column vectors, errors/undo, saved functions, expansion, Python and reload.');
  } finally {
